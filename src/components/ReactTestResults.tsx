...
import { ReactRunResult } from '../lib/reactRunner';

const ReactTestResults = ({ result }: { result?: ReactRunResult }) => {
    ...
    <div className="space-y-3">
      ...
      {result.results.map((item) => (
        <div key={item.name} className="rounded-xl border border-white/10 p-3">
          ...
          {item.error && <p className="mt-2 text-xs text-rose-300">Error: {item.error}</p>}
        </div>
      ))}
...
export default ReactTestResults;
