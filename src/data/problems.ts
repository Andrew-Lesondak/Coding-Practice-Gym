...
You may assume exactly one solution exists, and you may not use the same element twice.`,
    ...
    guidedStub: `function twoSum(nums: number[], target: number): number[] {\n  // Step 1: Initialize a map from number -> index so we can look up complements in O(1).\n  // TODO(step 1 start)\n  // Use a Map to store numbers we have seen and their indices.\n  // TODO(step 1 end)\n\n  // Step 2: Iterate through nums once, keeping the invariant that "seen" contains indices of previous elements.\n  // TODO(step 2 start)\n  // Loop over indices from 0 to nums.length - 1.\n  // TODO(step 2.1 start)\n  // Compute complement and check the map.\n  // TODO(step 2.1 end)\n  // TODO(step 2.2 start)\n  // Return indices when found, otherwise add to map.\n  // TODO(step 2.2 end)\n  // TODO(step 2 end)\n\n  // Step 3: If no solution exists, return an empty array (based on problem requirements).\n  // TODO(step 3 start)\n  // Return fallback.\n  // TODO(step 3 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'Map', message: 'Create a Map for number -> index.' },
      { stepIndex: 2, kind: 'regex', pattern: 'for\\s*\\(', message: 'Add a loop over the array.' },
      { stepIndex: 2.1, kind: 'regex', pattern: 'complement|target\\s*-', message: 'Compute the complement.' },
      { stepIndex: 2.2, kind: 'regex', pattern: '\\.set\\(', message: 'Store current number in the map.' },
      { stepIndex: 3, kind: 'regex', pattern: 'return\\s*\\[', message: 'Return the pair when found (or fallback).' }
    ],
    tests: {
      visible: [
      ...
      hidden: [
        ...
        { name: 'later pair', input: '[[1,5,1,5], 10]', expected: '[1,3]' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    guidedStub: `function isPalindrome(s: string): boolean {\n  // Step 1: Set up two pointers at the start and end of the string.\n  // TODO(step 1 start)\n  // Initialize left and right indices.\n  // TODO(step 1 end)\n\n  // Step 2: Define a helper to check if a character is alphanumeric.\n  // TODO(step 2 start)\n  // Create a small helper using a regex.\n  // TODO(step 2 end)\n\n  // Step 3: Move pointers inward in a loop, skipping non-alphanumeric characters.\n  // TODO(step 3 start)\n  // While left < right.\n  // TODO(step 3.1 start)\n  // Advance left/right while invalid.\n  // TODO(step 3.1 end)\n  // TODO(step 3.2 start)\n  // Compare lowercase characters and return false on mismatch.\n  // TODO(step 3.2 end)\n  // TODO(step 3 end)\n\n  // Step 4: If all pairs match, return true.\n  // TODO(step 4 start)\n  // Return true when loop finishes.\n  // TODO(step 4 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'left\\s*=|right\\s*=', message: 'Initialize left/right pointers.' },
      { stepIndex: 2, kind: 'regex', pattern: 'isAlpha|[a-z0-9]', message: 'Create an alphanumeric check.' },
      { stepIndex: 3, kind: 'regex', pattern: 'while\\s*\\(.*left.*right', message: 'Add the main while loop.' },
      { stepIndex: 3.1, kind: 'regex', pattern: 'isAlpha|test\\(', message: 'Skip non-alphanumeric characters.' },
      { stepIndex: 3.2, kind: 'regex', pattern: 'toLowerCase', message: 'Compare normalized characters.' },
      { stepIndex: 4, kind: 'regex', pattern: 'return\\s+true', message: 'Return true after successful scan.' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '["A man, a plan, a canal: Panama"]', expected: 'true' },
        { name: 'example 2', input: '["race a car"]', expected: 'false' },
        { name: 'mixed case', input: '["No \'x\' in Nixon"]', expected: 'true' }
      ],
      hidden: [
        { name: 'numbers', input: '["0P"]', expected: 'false' },
        { name: 'empty after filter', input: '["   "]', expected: 'true' },
        { name: 'symbols in between', input: '["ab_a"]', expected: 'true' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    guidedStub: `function lengthOfLongestSubstring(s: string): number {\n  // Step 1: Track last seen index for each character.\n  // TODO(step 1 start)\n  // Create a Map and window pointers.\n  // TODO(step 1 end)\n\n  // Step 2: Expand the window with a right pointer.\n  // TODO(step 2 start)\n  // Loop right across the string.\n  // TODO(step 2.1 start)\n  // If a character repeats inside the window, move left past its previous index.\n  // TODO(step 2.1 end)\n  // TODO(step 2.2 start)\n  // Update last seen index and best window length.\n  // TODO(step 2.2 end)\n  // TODO(step 2 end)\n\n  // Step 3: Return the best length.\n  // TODO(step 3 start)\n  // Return best.\n  // TODO(step 3 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'Map|lastSeen', message: 'Track last seen characters (map + pointers).' },
      { stepIndex: 2, kind: 'regex', pattern: 'for\\s*\\(.*right', message: 'Loop with a right pointer.' },
      { stepIndex: 2.1, kind: 'regex', pattern: 'left\\s*=', message: 'Move left when a repeat is found.' },
      { stepIndex: 2.2, kind: 'regex', pattern: '\\.set\\(|Math\\.max', message: 'Update last seen and best length.' },
      { stepIndex: 3, kind: 'regex', pattern: 'return\\s+best', message: 'Return the best window length.' }
    ],
    tests: {
      visible: [
      ...
      hidden: [
        ...
        { name: 'repeats inside', input: '["dvdf"]', expected: '3' },
        { name: 'mirror repeat', input: '["abba"]', expected: '2' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    guidedStub: `function isValidParentheses(s: string): boolean {\n  // Step 1: Set up a stack and a map of closing -> opening.\n  // TODO(step 1 start)\n  // Define stack and pairs map.\n  // TODO(step 1 end)\n\n  // Step 2: Iterate through the string.\n  // TODO(step 2 start)\n  // Loop over each character.\n  // TODO(step 2.1 start)\n  // Push opening brackets onto the stack.\n  // TODO(step 2.1 end)\n  // TODO(step 2.2 start)\n  // On closing, check the top of the stack and return false on mismatch.\n  // TODO(step 2.2 end)\n  // TODO(step 2 end)\n\n  // Step 3: At the end, stack must be empty.\n  // TODO(step 3 start)\n  // Return stack length check.\n  // TODO(step 3 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'stack|pairs', message: 'Create stack and pairs map.' },
      { stepIndex: 2, kind: 'regex', pattern: 'for\\s*\\(|for\\s+const', message: 'Iterate over characters.' },
      { stepIndex: 2.1, kind: 'regex', pattern: 'stack\\.push', message: 'Push opening brackets.' },
      { stepIndex: 2.2, kind: 'regex', pattern: 'stack\\.pop', message: 'Check closing brackets against stack.' },
      { stepIndex: 3, kind: 'regex', pattern: 'stack\\.length', message: 'Return empty-stack check.' }
    ],
    tests: {
      visible: [
      ...
      hidden: [
        { name: 'mixed', input: '["{[]}"]', expected: 'true' },
        { name: 'unbalanced', input: '["((("]', expected: 'false' },
        { name: 'crossed', input: '["([)]"]', expected: 'false' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    guidedStub: `function binarySearch(nums: number[], target: number): number {\n  // Step 1: Initialize left and right pointers.\n  // TODO(step 1 start)\n  // Set left = 0, right = nums.length - 1.\n  // TODO(step 1 end)\n\n  // Step 2: Loop while the search space is valid.\n  // TODO(step 2 start)\n  // While left <= right.\n  // TODO(step 2.1 start)\n  // Compute mid and compare to target.\n  // TODO(step 2.1 end)\n  // TODO(step 2.2 start)\n  // Move left or right to discard half.\n  // TODO(step 2.2 end)\n  // TODO(step 2 end)\n\n  // Step 3: If not found, return -1.\n  // TODO(step 3 start)\n  // Return -1.\n  // TODO(step 3 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'left\\s*=|right\\s*=', message: 'Initialize left/right.' },
      { stepIndex: 2, kind: 'regex', pattern: 'while\\s*\\(.*left.*right', message: 'Add the main while loop.' },
      { stepIndex: 2.1, kind: 'regex', pattern: 'mid', message: 'Compute and compare mid.' },
      { stepIndex: 2.2, kind: 'regex', pattern: 'left\\s*=|right\\s*=', message: 'Move bounds based on comparison.' },
      { stepIndex: 3, kind: 'regex', pattern: 'return\\s*-1', message: 'Return -1 when not found.' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[-1,0,3,5,9,12], 9]', expected: '4' },
        { name: 'example 2', input: '[[-1,0,3,5,9,12], 2]', expected: '-1' },
        { name: 'left edge', input: '[[-1,0,3,5,9,12], -1]', expected: '0' }
      ],
      hidden: [
        { name: 'single', input: '[[5], 5]', expected: '0' },
        { name: 'single miss', input: '[[1], 0]', expected: '-1' },
        { name: 'two elements', input: '[[2,4], 4]', expected: '1' }
      ]
    },
    metadata: {
  ...
  },
  {
    ...
    guidedStub: `type ListNode = { val: number; next: ListNode | null };\nfunction reverseList(head: ListNode | null): ListNode | null {\n  // Step 1: Initialize prev = null and curr = head.\n  // TODO(step 1 start)\n  // Set up pointers.\n  // TODO(step 1 end)\n\n  // Step 2: Loop while curr exists.\n  // TODO(step 2 start)\n  // While curr is not null.\n  // TODO(step 2.1 start)\n  // Save next pointer before overwriting.\n  // TODO(step 2.1 end)\n  // TODO(step 2.2 start)\n  // Reverse link, then advance prev/curr.\n  // TODO(step 2.2 end)\n  // TODO(step 2 end)\n\n  // Step 3: After loop, prev is new head.\n  // TODO(step 3 start)\n  // Return prev.\n  // TODO(step 3 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'prev|curr', message: 'Initialize prev and curr pointers.' },
      { stepIndex: 2, kind: 'regex', pattern: 'while\\s*\\(.*curr', message: 'Add the main loop.' },
      { stepIndex: 2.1, kind: 'regex', pattern: 'next', message: 'Save the next pointer.' },
      { stepIndex: 2.2, kind: 'regex', pattern: 'curr\\.next\\s*=\\s*prev', message: 'Reverse the link and advance.' },
      { stepIndex: 3, kind: 'regex', pattern: 'return\\s+prev', message: 'Return the new head.' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[1,2,3,4,5]]', expected: '[5,4,3,2,1]' },
        { name: 'single', input: '[[1]]', expected: '[1]' },
        { name: 'two nodes', input: '[[1,2]]', expected: '[2,1]' }
      ],
      hidden: [
        { name: 'empty', input: '[[]]', expected: '[]' },
        { name: 'three nodes', input: '[[1,2,3]]', expected: '[3,2,1]' },
        { name: 'four nodes', input: '[[1,2,3,4]]', expected: '[4,3,2,1]' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'return\\s+0|!root', message: 'Handle the empty tree case.' },
      { stepIndex: 2, kind: 'regex', pattern: 'maxDepth\\(root\\.left\\)', message: 'Recurse into left subtree.' },
      { stepIndex: 3, kind: 'regex', pattern: 'maxDepth\\(root\\.right\\)', message: 'Recurse into right subtree.' },
      { stepIndex: 4, kind: 'regex', pattern: 'Math\\.max', message: 'Return 1 + max(left, right).' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[3,9,20,null,null,15,7]]', expected: '3' },
        { name: 'single', input: '[[1]]', expected: '1' },
        { name: 'left skew', input: '[[1,2]]', expected: '2' }
      ],
      hidden: [
        { name: 'empty', input: '[[]]', expected: '0' },
        { name: 'balanced', input: '[[1,2,3,4,5,6,7]]', expected: '3' },
        { name: 'uneven', input: '[[1,2,3,4,null,null,5]]', expected: '3' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    guidedStub: `type TreeNode = { val: number; left: TreeNode | null; right: TreeNode | null };\nfunction levelOrder(root: TreeNode | null): number[][] {\n  // Step 1: Handle empty tree.\n  // TODO(step 1 start)\n  // Return [] when root is null.\n  // TODO(step 1 end)\n\n  // Step 2: Initialize queue with root and result array.\n  // TODO(step 2 start)\n  // Create queue and result.\n  // TODO(step 2 end)\n\n  // Step 3: While queue not empty, process one level at a time.\n  // TODO(step 3 start)\n  // Loop while queue has nodes.\n  // TODO(step 3.1 start)\n  // Capture level size and initialize a level array.\n  // TODO(step 3.1 end)\n  // TODO(step 3.2 start)\n  // Pop size nodes and enqueue children.\n  // TODO(step 3.2 end)\n  // TODO(step 3 end)\n\n  // Step 4: Return result.\n  // TODO(step 4 start)\n  // Return result.\n  // TODO(step 4 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'return\\s*\\[\\]', message: 'Return [] for empty tree.' },
      { stepIndex: 2, kind: 'regex', pattern: 'queue|result', message: 'Initialize queue and result.' },
      { stepIndex: 3, kind: 'regex', pattern: 'while\\s*\\(.*queue', message: 'Process while queue has nodes.' },
      { stepIndex: 3.1, kind: 'regex', pattern: 'size|level', message: 'Capture level size and init array.' },
      { stepIndex: 3.2, kind: 'regex', pattern: 'queue\\.push', message: 'Enqueue children.' },
      { stepIndex: 4, kind: 'regex', pattern: 'return\\s+result', message: 'Return result.' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[3,9,20,null,null,15,7]]', expected: '[[3],[9,20],[15,7]]' },
        { name: 'single', input: '[[1]]', expected: '[[1]]' },
        { name: 'two levels', input: '[[1,2,3,4,5]]', expected: '[[1],[2,3],[4,5]]' }
      ],
      hidden: [
        { name: 'empty', input: '[[]]', expected: '[]' },
        { name: 'right chain', input: '[[1,null,2,3]]', expected: '[[1],[2],[3]]' },
        { name: 'left chain', input: '[[1,2,null,3]]', expected: '[[1],[2],[3]]' }
      ]
    },
    metadata: {
      timeComplexity: 'O(n)',
  ...
  },
  {
    ...
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'push\\(|bubbleUp', message: 'Implement heap push + bubbleUp.' },
      { stepIndex: 2, kind: 'regex', pattern: 'pop\\(|bubbleDown', message: 'Implement heap pop + bubbleDown.' },
      { stepIndex: 3, kind: 'regex', pattern: 'peek\\(|size\\(', message: 'Add peek() and size().' },
      { stepIndex: 4, kind: 'regex', pattern: 'for\\s*\\(|for\\s+const', message: 'Loop through nums.' },
      { stepIndex: 4.1, kind: 'regex', pattern: 'heap\\.push', message: 'Push into heap.' },
      { stepIndex: 4.2, kind: 'regex', pattern: 'heap\\.pop|size\\(\\)\\s*>\\s*k', message: 'Trim heap to size k.' },
      { stepIndex: 5, kind: 'regex', pattern: 'heap\\.peek', message: 'Return heap.peek().' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[3,2,1,5,6,4], 2]', expected: '5' },
        { name: 'example 2', input: '[[3,2,3,1,2,4,5,5,6], 4]', expected: '4' },
        { name: 'example 3', input: '[[7,10,4,3,20,15], 3]', expected: '10' }
      ],
      hidden: [
        { name: 'single', input: '[[1], 1]', expected: '1' },
        { name: 'two elements', input: '[[2,1], 2]', expected: '1' },
        { name: 'k at end', input: '[[3,2,1,5,6,4], 6]', expected: '1' }
      ]
    },
    metadata: {
  ...
  },
  {
    ...
    guidedStub: `function numIslands(grid: string[][]): number {\n  // Step 1: Capture grid dimensions and visited tracking.\n  // TODO(step 1 start)\n  // Define rows, cols, visited.\n  // TODO(step 1 end)\n\n  // Step 2: Write a DFS helper that marks land as visited.\n  // TODO(step 2 start)\n  // Implement dfs with bounds and water checks.\n  // TODO(step 2 end)\n\n  // Step 3: Walk all cells and launch DFS on unvisited land.\n  // TODO(step 3 start)\n  // Loop over rows.\n  // TODO(step 3.1 start)\n  // Loop over columns.\n  // TODO(step 3.1 end)\n  // TODO(step 3.2 start)\n  // If cell is unvisited land, increment count and call dfs.\n  // TODO(step 3.2 end)\n  // TODO(step 3 end)\n\n  // Step 4: Return the count.\n  // TODO(step 4 start)\n  // Return count.\n  // TODO(step 4 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'rows|cols|visited', message: 'Capture dimensions + visited.' },
      { stepIndex: 2, kind: 'regex', pattern: 'dfs', message: 'Define the DFS helper.' },
      { stepIndex: 3, kind: 'regex', pattern: 'for\\s*\\(.*r', message: 'Loop over rows.' },
      { stepIndex: 3.1, kind: 'regex', pattern: 'for\\s*\\(.*c', message: 'Loop over columns.' },
      { stepIndex: 3.2, kind: 'regex', pattern: 'dfs\\(|grid\\[r\\]\\[c\\]', message: 'Start DFS on unvisited land.' },
      { stepIndex: 4, kind: 'regex', pattern: 'return\\s+count', message: 'Return the island count.' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[["1","1","0"],["1","0","0"],["0","0","1"]]]', expected: '2' },
        { name: 'all land', input: '[[["1","1"],["1","1"]]]', expected: '1' },
        { name: 'single land', input: '[[["1"]]]', expected: '1' }
      ],
      hidden: [
        { name: 'all water', input: '[[["0","0"],["0","0"]]]', expected: '0' },
        { name: 'row islands', input: '[[["1","0","1","0"]]]', expected: '2' },
        { name: 'block islands', input: '[[["1","1","0","1"],["1","1","0","0"]]]', expected: '2' }
      ]
    },
    metadata: {
  ...
  },
  {
    ...
    guidedStub: `function climbStairs(n: number): number {\n  // Step 1: Handle small n directly.\n  // TODO(step 1 start)\n  // Return n when n <= 2.\n  // TODO(step 1 end)\n\n  // Step 2: Initialize dp for ways to reach step 1 and 2.\n  // TODO(step 2 start)\n  // Set prev = 1, curr = 2.\n  // TODO(step 2 end)\n\n  // Step 3: Iterate from step 3 to n.\n  // TODO(step 3 start)\n  // Loop i from 3 to n.\n  // TODO(step 3.1 start)\n  // Compute next = prev + curr.\n  // TODO(step 3.1 end)\n  // TODO(step 3.2 start)\n  // Shift prev and curr forward.\n  // TODO(step 3.2 end)\n  // TODO(step 3 end)\n\n  // Step 4: Return curr as answer.\n  // TODO(step 4 start)\n  // Return curr.\n  // TODO(step 4 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'n\\s*<=\\s*2|return\\s+n', message: 'Handle small n directly.' },
      { stepIndex: 2, kind: 'regex', pattern: 'prev|curr', message: 'Initialize rolling values.' },
      { stepIndex: 3, kind: 'regex', pattern: 'for\\s*\\(.*i\\s*=\\s*3', message: 'Loop from 3 to n.' },
      { stepIndex: 3.1, kind: 'regex', pattern: 'next\\s*=\\s*prev\\s*\\+\\s*curr', message: 'Compute next value.' },
      { stepIndex: 3.2, kind: 'regex', pattern: 'prev\\s*=|curr\\s*=', message: 'Shift prev/curr.' },
      { stepIndex: 4, kind: 'regex', pattern: 'return\\s+curr', message: 'Return the final count.' }
    ],
    tests: {
      visible: [
  ...
  },
  {
    ...
    guidedStub: `function mergeIntervals(intervals: number[][]): number[][] {\n  // Step 1: Handle empty input and sort by start time.\n  // TODO(step 1 start)\n  // Return [] if needed and sort intervals.\n  // TODO(step 1 end)\n\n  // Step 2: Seed the result with the first interval.\n  // TODO(step 2 start)\n  // Initialize result array.\n  // TODO(step 2 end)\n\n  // Step 3: Iterate through intervals and compare with last merged.\n  // TODO(step 3 start)\n  // Loop from index 1.\n  // TODO(step 3.1 start)\n  // Compare current with last merged.\n  // TODO(step 3.1 end)\n  // TODO(step 3.2 start)\n  // Merge if overlapping, otherwise append.\n  // TODO(step 3.2 end)\n  // TODO(step 3 end)\n\n  // Step 4: Return merged result.\n  // TODO(step 4 start)\n  // Return result.\n  // TODO(step 4 end)\n\n  return;\n}`,
    stepChecks: [
      { stepIndex: 1, kind: 'regex', pattern: 'sort\\(', message: 'Sort intervals by start.' },
      { stepIndex: 2, kind: 'regex', pattern: 'result', message: 'Initialize the result array.' },
      { stepIndex: 3, kind: 'regex', pattern: 'for\\s*\\(.*i\\s*=\\s*1', message: 'Loop through intervals.' },
      { stepIndex: 3.1, kind: 'regex', pattern: 'current\\[0\\]|last\\[1\\]', message: 'Compare current vs last.' },
      { stepIndex: 3.2, kind: 'regex', pattern: 'Math\\.max|result\\.push', message: 'Merge or append intervals.' },
      { stepIndex: 4, kind: 'regex', pattern: 'return\\s+result', message: 'Return merged result.' }
    ],
    tests: {
      visible: [
        { name: 'example 1', input: '[[[1,3],[2,6],[8,10],[15,18]]]', expected: '[[1,6],[8,10],[15,18]]' },
        { name: 'touching', input: '[[[1,4],[4,5]]]', expected: '[[1,5]]' },
        { name: 'unsorted', input: '[[[1,4],[0,2],[3,5]]]', expected: '[[0,5]]' }
      ],
      hidden: [
        { name: 'single', input: '[[[1,4]]]', expected: '[[1,4]]' },
        { name: 'overlap nested', input: '[[[1,4],[2,3]]]', expected: '[[1,4]]' },
        { name: 'no overlap', input: '[[[1,4],[5,6]]]', expected: '[[1,4],[5,6]]' }
      ]
    },
    metadata: {
